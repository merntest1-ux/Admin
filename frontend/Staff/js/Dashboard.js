  ...
  async function loadGradeChart() {
    try {
      ...
      if (response.success) {
        ...
        const gradeCounts = {};
        referrals.forEach(ref => {
          // Normalize grade: extract number from "Grade 1", "grade 1", or "1"
          const gradeNum = ref.grade.toString().match(/\d+/)?.[0];
          if (gradeNum) {
      ...
      <div class="ai-prescription-modal-content">
        <div class="ai-prescription-modal-header">
          <div class="header-content">
            ...
            <h2 id="topReferral">Top Referral: ${topReferral.category} (${topReferral.count} mentions)</h2>
...
function formatAISolutionForTextarea(prescription) {
  ...
  // Header
  formatted += `WEEKLY AI PRESCRIPTION\n`;
  formatted += `Week ${week} (${year}) - Generated on ${date.toLocaleString()}\n`;
  formatted += `${'='.repeat(70)}\n\n`;
  
  // Issue
  formatted += `TRENDING ISSUE:\n`;
  formatted += `${p.issue}\n\n`;
  
  // Severity
  formatted += `SEVERITY LEVEL: ${p.solution.severity.toUpperCase()}\n`;
  formatted += `${'='.repeat(70)}\n\n`;
  ...
  // Solutions
  formatted += `RECOMMENDED SOLUTIONS:\n\n`;
  ...
  if (p.solution.quick_wins && p.solution.quick_wins.length > 0) {
    formatted += `${'='.repeat(70)}\n\n`;
    formatted += `‼️ QUICK WINS (Immediate Actions):\n\n`;
  ...
  // Footer
  formatted += `${'='.repeat(70)}\n`;
  formatted += `\nThis prescription is valid for the current week.\n`;
  formatted += ` Review and implement solutions based on your school's context.\n`;
  formatted += ` Check back next week for updated recommendations.`;
