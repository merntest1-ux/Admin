// Adviser ProfileSettings.js - Updated Avatar Styling
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // --------------------------
  // GENERATE AVATAR WITH INITIAL - UPDATED STYLING
  // --------------------------
  function generateAvatar(name, size = 96) {
    const initial = name ? name.charAt(0).toUpperCase() : '?';
    const fontSize = size === 96 ? 48 : (size === 80 ? 40 : 20);
  ...
  function displayUserProfile(user) {
    ...
    if (user.profilePhoto || user.avatar || user.photo) {
      ...
      // Update main profile pic (96px)
      if (profilePicContainer) {
        const fallbackAvatar = generateAvatar(userName, 96);
        profilePicContainer.innerHTML = `
          <img src="${photoUrl}" 
               alt="${userName}" 
               style="width: 96px; height: 96px; border-radius: 50%; object-fit: cover; border: 3px solid #10b981; box-shadow: 0 4px 12px rgba(16, 185, 129, 0.3);"
      ...
      if (headerProfileButton) {
        ...
        headerProfileButton.innerHTML = `
          <img src="${photoUrl}" 
               alt="${userName}"
               style="width: 40px; height: 40px; border-radius: 50%; object-fit: cover; cursor: pointer; border: 2px solid #10b981; box-shadow: 0 2px 8px rgba(16, 185, 129, 0.3);"
